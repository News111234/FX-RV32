...
module mem_wb_reg (
    ...
    // ========== 内存异常传递 (MEM → WB → CSR) ==========
    input  wire        mem_exception_i,       // 内存异常标志
    input  wire [31:0] mem_exception_addr_i,  // 异常地址 (用于mtval)
    input  wire        mem_misalign_i,        // 未对齐异常 (mcause=4/6)
    input  wire        mem_range_err_i,       // 范围越界异常 (mcause=5/7)
    input  wire        mem_mem_we_i,          // 是否为store指令 (区分mcause)
    ...
    output reg  [31:0] wb_exception_addr_o,
    output reg         wb_exception_misalign_o,  // 0=misalign, 1=access-fault
    output reg         wb_exception_load_o       // 1=load, 0=store
);
...
always @(posedge clk_i ) begin
    if (!rst_n_i) begin
        ...
        wb_mem_re_o          <= 1'b0;
        wb_exception_o       <= 1'b0;
        wb_exception_addr_o  <= 32'b0;
        wb_exception_misalign_o <= 1'b0;
        wb_exception_load_o  <= 1'b0;
    end
        ...
        wb_mem_re_o          <= mem_mem_re_i;
        wb_exception_o       <= mem_exception_i;
        wb_exception_addr_o  <= mem_exception_addr_i;
        wb_exception_misalign_o <= mem_misalign_i;
        wb_exception_load_o  <= mem_mem_re_i;  // load=1, store=0
    end
end
