// rtl/core/core_top.v - RISC-V CPU核心 (单Bank影子寄存器, TVLSI基线)
`timescale 1ns/1ps

module core_top #(
    parameter SYNC_INST_MEM   = 0    // 1=BRAM(sync read), 0=ROM(combinational read)
) (
    // ========== 系统接口 ==========
    input  wire        clk_i,
    input  wire        rst_n_i,
...
// 外部中断子优先级选择器位于 interrupt_controller 内部 (由 meipr CSR 配置)
wire intr_external;   // 外部中断有效标志 (interrupt_controller 输出, 供 csr_regfile mip[11])
...
wire        mem_bus_ready;
wire        mem_exception;
wire [31:0] mem_exception_addr;
wire        mem_misalign_flag;
wire        mem_range_err;
wire        mem_exception_we;

// MEM/WB 异常输出
wire        wb_exception;
wire [31:0] wb_exception_addr;
wire        wb_exception_misalign;
wire        wb_exception_load;

// 异常 CSR 写信号 (与中断写信号 OR 后送入 csr_regfile)
wire        exc_csr_mcause_we;
wire [31:0] exc_csr_mcause_data;
wire        exc_csr_mepc_we;
wire [31:0] exc_csr_mepc_data;
wire        exc_csr_mstatus_we;
wire [31:0] exc_csr_mstatus_data;

// 前向声明: 中断/CSR 信号 (由中断流水线和 CSR 寄存器文件驱动)
wire [31:0] mstatus;
wire        int_pipe_csr_mepc_we;
wire [31:0] int_pipe_csr_mepc_data;
wire        int_pipe_csr_mcause_we;
wire [31:0] int_pipe_csr_mcause_data;
wire        int_pipe_csr_mstatus_we;
wire [31:0] int_pipe_csr_mstatus_data;

// 异常 mcause 生成: misalign+load=4, misalign+store=6, range+load=5, range+store=7
assign exc_csr_mcause_we   = wb_exception;
assign exc_csr_mcause_data  = {1'b0, 26'b0, 3'b0,
                                wb_exception_misalign ? 1'b0 : 1'b1,  // bit[1]: 0=misalign, 1=access-fault
                                wb_exception_load     ? 1'b0 : 1'b1}; // bit[0]: 0=load, 1=store

assign exc_csr_mepc_we      = wb_exception;
assign exc_csr_mepc_data    = wb_pc_plus4;  // 简化: mepc 用 WB PC+4, 实际应为 MEM 阶段 PC
assign exc_csr_mstatus_we   = wb_exception;
assign exc_csr_mstatus_data = {mstatus[31:4], 1'b0, mstatus[2:0]}; // 关闭 MIE

// OR 合并中断和异常的 CSR 写
wire pipe_csr_mepc_we    = int_pipe_csr_mepc_we    || exc_csr_mepc_we;
wire [31:0] pipe_csr_mepc_data   = exc_csr_mepc_we    ? exc_csr_mepc_data    : int_pipe_csr_mepc_data;
wire pipe_csr_mcause_we  = int_pipe_csr_mcause_we  || exc_csr_mcause_we;
wire [31:0] pipe_csr_mcause_data = exc_csr_mcause_we  ? exc_csr_mcause_data  : int_pipe_csr_mcause_data;
wire pipe_csr_mstatus_we = int_pipe_csr_mstatus_we || exc_csr_mstatus_we;
wire [31:0] pipe_csr_mstatus_data= exc_csr_mstatus_we ? exc_csr_mstatus_data : int_pipe_csr_mstatus_data;
...
wire        interrupt_flush_all = interrupt_flush_pipe || wb_exception;  // 中断 + 异常都触发刷新
...
hazard_unit #(
    ...
    .jump_taken_i     (ex_jump_taken),
    .interrupt_taken_i(interrupt_taken_pipe),
    .interrupt_flush_i(interrupt_flush_all),
    .stall_if_o       (stall_if),
    .stall_id_o       (stall_id),
    .flush_if_o       (flush_if),
    .flush_id_o       (flush_id),
...
mem_top u_mem_top (
    ...
    .mem_misalign_flag_o   (mem_misalign_flag),
    .mem_range_err_o       (mem_range_err),
    .mem_exception_we_o    (mem_exception_we)
);
...
mem_wb_reg u_mem_wb_reg (
    ...
    .mem_misalign_i         (mem_misalign_flag),
    .mem_range_err_i        (mem_range_err),
    .mem_mem_we_i           (ex_mem_mem_we),
    .wb_mem_re_o            (mem_wb_mem_re),
    .wb_exception_o         (wb_exception),
    .wb_exception_addr_o    (wb_exception_addr),
    .wb_exception_misalign_o(wb_exception_misalign),
    .wb_exception_load_o    (wb_exception_load),
...
csr_regfile u_csr_regfile (
    ...
    .csr_mtval_we_i     (wb_exception),
    .csr_mtval_data_i   (wb_exception_addr),
    .intr_software_i    (intr_software_i),
    .intr_timer_i       (intr_timer_i),
    .intr_external_i    (intr_external),   // 来自 interrupt_controller 的选中标志
    .mtvec_o            (mtvec),
    .mepc_o             (mepc),
    .mcause_o           (mcause),
    .mie_o              (mie),
...
interrupt_controller u_interrupt_controller (
    ...
    .intr_software_i    (intr_software_i),
    .intr_timer_i       (intr_timer_i),
    .extr_gpio_i        (extr_gpio_i),
    .extr_spi_i         (extr_spi_i),
    .extr_i2c_i         (extr_i2c_i),
    .mie_i              (mie),
    .mip_i              (mip),
    .mstatus_i          (mstatus),
    .mtvec_i            (mtvec),
    ...
    .intr_external_o    (intr_external),
    .current_priority_o (current_priority),
    .new_priority_o     (new_priority)
);

// ========== 多Bank影子寄存器控制器 (已移除: FX-RV32 为单Bank设计, 不支持中断嵌套) ==========
// ========== 中断流水线 (单Bank版本) ==========
interrupt_pipeline u_interrupt_pipeline (
    ...
    .mstatus_i          (mstatus),
    .csr_mepc_we_o      (int_pipe_csr_mepc_we),
    .csr_mepc_data_o    (int_pipe_csr_mepc_data),
    .csr_mcause_we_o    (int_pipe_csr_mcause_we),
    .csr_mcause_data_o  (int_pipe_csr_mcause_data),
    .csr_mstatus_we_o   (int_pipe_csr_mstatus_we),
    .csr_mstatus_data_o (int_pipe_csr_mstatus_data),
