...
//   6. 根据wb_sel_i选择最终结果(ALU结果 / 内存数据 / PC+4 / CSR结果),把wb_sel_i给透传下去
...
//   │ LSB 可能为 1？ │ 不会                                          │ 会                                           │
//   ├────────────────┼───────────────────────────────────────────────┼──────────────────────────────────────────────┤
//   │ 为什么         │ J-type 立即数 bit0 恒为 0，PC 恒为 4 字节对齐 │ rs1 是寄存器值，程序员可能放进奇数地址       │
//   ├────────────────┼───────────────────────────────────────────────┼──────────────────────────────────────────────┤
//   │ 需要 & ~1？    │ 不需要（做也无害）                            │ 必须，否则跳转到一个不对齐的地址取指令会出错 │
// ============================================================================
module ex_top (
    // ========== 系统接口 ==========
