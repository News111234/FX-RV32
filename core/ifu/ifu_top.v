...
//
// 参数:
//   SYNC_INST_MEM = 0 (默认): 组合指令存储器 (inst_rom), 读零延迟
//                              pc = pc_value, 无延迟补偿
// ============================================================================
module ifu_top #(
    parameter SYNC_INST_MEM = 0  // 0=ROM(combinational read)
) (
    // ========== 时钟和复位 ==========
    input  wire        clk,
    input  wire        rst_n,
...
// 分支和跳转互斥 (同一EX阶段, 不可能同时触发), 合并为同一优先级
wire        ctrl_flow_taken  = branch_taken_i | jump_taken_i;
wire [31:0] ctrl_flow_target = branch_taken_i ? branch_target_i : jump_target_i;
assign next_pc = (!rst_n)               ? 32'h0 :
                 (intr_take_now_i)       ? intr_target_i :
                 (interrupt_taken_i)     ? pc_value + 32'h4 :
                 (ctrl_flow_taken)       ? ctrl_flow_target :
                 (stall_i)              ? pc_value :
                 pc_value + 32'h4;
