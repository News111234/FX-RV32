...
// 默认: GPIO=11 > SPI=7 > I2C=3, 同时触发时取最高优先级源
// ==========================================================================
wire [3:0] gpio_prio = meipr[3:0];
wire [3:0] spi_prio  = meipr[7:4];
wire [3:0] i2c_prio  = meipr[11:8];

wire       gpio_active = gpio_interrupt && (gpio_prio > 4'd0);
wire       spi_active  = spi_interrupt  && (spi_prio  > 4'd0);
wire       i2c_active  = i2c_interrupt  && (i2c_prio  > 4'd0);
...
always @(posedge clk_i) begin
    if (rst_n_i) begin
        ...
        if (stall_if || stall_id || flush_if || flush_id || 
            intr_flush_id) begin
            $display("[%0t] Cycle %0d: Pipeline Control: stall_if=%b, stall_id=%b, flush_if=%b, flush_id=%b, intr_flush_id=%b",
                    $time, cycle_counter, stall_if, stall_id, flush_if, flush_id, intr_flush_id);
        end
