...
// 描述:
//   本模块具有内存访问操作的预检查:
//   1. 地址对齐检查: LW/SW地址需按4字节对齐，LH/SH地址需按2字节对齐
...
// ========== 地址对齐检查 ==========
wire misalign = (mem_width_i == 3'b010 && alu_result_i[1:0] != 2'b00) || // LW/SW 需 4 字节对齐
                (mem_width_i == 3'b001 && alu_result_i[0]   != 1'b0);    // LH/SH 需 2 字节对齐

// ========== 地址范围检查 ==========
wire in_range = (alu_result_i[31:28] == 4'h0);  // 有效地址范围: 0x0000_0000 ~ 0x0FFF_FFFF

// ========== 地址有效性 = 对齐 AND 范围内 ==========
wire addr_valid = !misalign && in_range;

// ========== 输出信号 ==========
assign mem_addr_o   = alu_result_i;
assign mem_wdata_o  = wdata_i;          // data_ram 内部处理写数据字节/半字写入
assign mem_we_o     = mem_we_i && addr_valid;
assign mem_re_o     = mem_re_i && addr_valid;
assign mem_width_o  = mem_width_i;

// ========== 异常信号 ==========
assign mem_misalign_o = misalign;
assign mem_error_o    = (mem_we_i || mem_re_i) && !in_range;
