...
//   - mie:     机器中断使能寄存器
//   - mip:     机器中断挂起寄存器
//   - mipr:    机器中断优先级寄存器 (4-bit per source: MEI[11:8] MTI[7:4] MSI[3:0])
//   - meipr:   外部中断子优先级寄存器 (4-bit per source: I2C[11:8] SPI[7:4] GPIO[3:0])
...
module csr_regfile (
    ...
    // ========== 异常写端口 (新增 — 内存异常用) ==========
    input  wire        csr_mtval_we_i,     // 写 mtval 使能 (内存异常时)
    input  wire [31:0] csr_mtval_data_i,   // 写入 mtval 的值 (异常地址)
    ...
    output reg  [31:0] mipr_o,            // 机器中断优先级
    output reg  [31:0] meipr_o            // 外部中断子优先级
...
localparam CSR_MIPR      = 12'hBC0;   // 中断优先级 (machine-mode, 自定义)
localparam CSR_MEIPR     = 12'hBC1;   // 外部中断子优先级 (machine-mode, 自定义)
...
reg [31:0] mtval;
reg [31:0] mip;
reg [31:0] mipr;    // MEI[11:8] MTI[7:4] MSI[3:0], 默认 11/7/3
reg [31:0] meipr;   // I2C[11:8] SPI[7:4] GPIO[3:0], 默认 11/7/3
// 扩展 64 位计数器
reg [63:0] mcycle;
reg [63:0] minstret;
...
always @(posedge clk_i) begin
    if (!rst_n_i) begin
        ...
        mtval     <= 32'b0;
        mip       <= 32'b0;
        mipr      <= 32'h00000B73;  // MEI=11, MTI=7, MSI=3 (与硬编码默认一致)
        meipr     <= 32'h00000B73;  // GPIO=11, SPI=7, I2C=3 (与硬编码默认一致)
        mcycle    <= 64'b0;
        minstret  <= 64'b0;
    end else begin
        // 更新 mip: 使用组合逻辑计算的值
        ...
        if (csr_inst_we_i) begin
            case (csr_inst_waddr_i)
                ...
                CSR_MIPR:     mipr      <= csr_inst_wdata_i;
                CSR_MEIPR:    meipr     <= csr_inst_wdata_i;
...
always @(*) begin
    case (csr_addr_i)
        ...
        CSR_MIPR:      csr_rdata_o = mipr;
        CSR_MEIPR:     csr_rdata_o = meipr;
